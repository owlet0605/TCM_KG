    ...
    <el-row>
      ...
      <el-col :span="22" class="context">
        ...
        <el-row class="comment">
          <el-text v-if="comment.commentForId===comment.commentTopId">
...
export default {
  name: "SubCommentItem",
  components: {Pointer, Delete, Apple},
  props: ['comment'],
  data() {
    return {
      user: {
  ...
  created() {
    ...
    if(this.comment.commentForId!==this.comment.commentTopId){
      getComment(this.comment.commentForId).then(res=>{
        if(res.data.code===200){
          this.forComment.commentId=res.data.object.commentId
          this.forComment.commentUserId=res.data.commentUserId
          getUser(this.forComment.commentUserId).then(res=>{
            if(res.data.code===200){
              this.forComment.commentUserName=res.data.object.userName
            }
          }).catch(e=>{
            console.log(e)
          })
  ...
  methods: {
    ...
    commentIt: function (){
      this.myComment.commentUserId=this.$store.getter['user'].userId
      this.myComment.commentForId=this.comment.commentId
