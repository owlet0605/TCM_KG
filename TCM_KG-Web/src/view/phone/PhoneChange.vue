            ...
            <el-form-item>
              <el-col>
                <el-input v-model="phone">
                  <template #prepend>
                    <el-button disabled>现手机号码</el-button>
                  </template>
                </el-input>
              </el-col>
            </el-form-item>
...
import {changeUserPhone, confirmPhoneCode, sendPhoneCode} from "@/api/user";
  ...
  methods: {
    ...
    change: function (){
      confirmPhoneCode(this.phone, this.code).then(res=>{
        if(res.data.code===200){
          changeUserPhone(this.$store.getters['user'].userId,this.phone).then(res=>{
            if(res.data.code===200){
