    ...
    <el-main>
      <div class="detail">
        ...
        <div class="form">
          ...
          <el-form label-width="100px" label-position="left" style="max-width: 460px">
            <el-form-item>
              <el-col>
                <el-input v-model="phone" disabled>
                  <template #prepend>
                    <el-button disabled>原手机号码</el-button>
                  </template>
                </el-input>
              </el-col>
            </el-form-item>
            ...
            <el-form-item>
              <el-col><el-button type="primary" @click="confirm">下一步</el-button></el-col>
            </el-form-item>
          </el-form>
        </div>
      </div>
...
import {confirmPhoneCode, sendPhoneCode} from "@/api/user";
...
export default {
  ...
  data() {
    return{
      phone: this.$store.getters['user'].userPhone,
      code: ""
    }
  },
  methods: {
    send: function (){
      sendPhoneCode(this.phone).then(res=> {
        if (res.data.code === 200) {
          ElNotification({
            title: '发送成功',
            message: '手机验证码发送成功，有效期为5分钟，注意查收',
            type: 'success',
          })
        } else {
          ElNotification({
    ...
    confirm: function (){
      confirmPhoneCode(this.phone, this.code).then(res=>{
        if(res.data.code===200){
          this.$router.push({
            name: 'PhoneChange'
          })
